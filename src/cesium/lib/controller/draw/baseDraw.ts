...
export class BaseDraw3D {
    ...
    constructor(viewer, drawType: DrawType) {
        this.viewer = viewer;
        this.drawType = drawType;
        this.handler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas);
        this.tooltip = createTooltip(<HTMLDivElement>viewer.container.firstElementChild);
    ...
    drawRectangleFn(): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            this.handler.setInputAction(click => {
                if (!this.moveStatus) {
                    this.tooltip.showAt(click.position, '<p>请拖动鼠标绘制区域范围</p>');
                    startScreenCoordinate = JSON.parse(JSON.stringify(click.position));
                    this.startLnglat = screenToTerrainGraphical(this.viewer, click.position);
                }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.handler.setInputAction(leftClick => {
                if (!this.moveStatus) {
                    ...
                    const { lngLat, cartographicHeight } = screenCoordinatesToDegrees(
                        this.viewer,
                        leftClick.position
                    );
                    ...
                    this.tooltip.showAt(leftClick.position, '<p>请拖动鼠标绘制区域范围</p>');
                    ...
                    this.entity = this.viewer.entities.add({
                        ...
                        polyline: {
                            positions: new Cesium.CallbackProperty(() => {
                                return Cesium.Cartesian3.fromDegreesArray(this.westSouthEastNorth);
                            }),
            ...
            this.handler.setInputAction(move => {
                if (!this.moveStatus) {
                    this.tooltip.showAt(move.endPosition, '<p>请单击鼠标左键开始绘制区域</p>');
                } else {
                    this.tooltip.showAt(move.endPosition, '<p>请单击鼠标右键结束绘制</p>');
                    const { lngLat } = screenCoordinatesToDegrees(this.viewer, move.endPosition);
            ...
            this.handler.setInputAction(event => {
                ...
                const { northwest, southeast } = toRectangleLonLat(this.startLnglat, endLngLat);
                ...
                if (!this.moveStatus) {
                    ...
                    this.entity = this.viewer.entities.add({
                        position: new Cesium.CallbackProperty(() => {
                            return new Cesium.Cartesian3.fromDegrees(...this.startLnglat);
                        }, false),
                        name: 'circle',
                        id: this.drawData.id,
                        ellipse: {
                    ...
                    this.entity.ellipse.semiMajorAxis = new Cesium.CallbackProperty(() => {
                        return radius;
                    }, false);
                    this.entity.ellipse.semiMinorAxis = new Cesium.CallbackProperty(() => {
                        return radius;
                    }, false);

                    const ray = this.viewer.camera.getPickRay(leftClick.position);
            ...
            this.handler.setInputAction(event => {
                ...
                this.viewer.camera.flyTo({
                    ...
                    complete: () => {
                        ...
                        const circleSideLatLng = [coordinates[0], coordinates[1]];
                        ...
                        const centerLatLng = screenCoordinatesToDegrees(this.viewer, {
                            x: clientCenterW,
                            y: clientCenterH
                        });

                        // 求出指定方位经纬
                        const targetLatLng = getTargetLatLng(centerLatLng.lngLat, circleSideLatLng);

                        // 指定经纬转换屏幕坐标
                        const northPosition = this.toScreenCoordinates(targetLatLng, startHeight);
    ...
    toScreenCoordinates(latLng, height) {
        const position = Cesium.Cartesian3.fromDegrees(latLng[0], latLng[1], height);
    ...
    getCircleBoundary(centerLatLng, circleSideLatLng) {
        ...
        // 南方向经纬, lat作为东南角的lat
        const south = getTargetLatLng(centerLatLng.lngLat, circleSideLatLng, 180);
        // 获取西北角的经纬
        const westNorth = getTargetLatLng(centerLatLng.lngLat, [east[0], south[1]], -45);
