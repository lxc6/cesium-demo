...
export class MeasureSuperMap extends ChainNodeExecutor {
    ...
    handlers = {
        [Cesium.MeasureMode.Distance]: ({
            ...
            if (this.activeMeasure) {
                ...
                positions.forEach((position, index) => {
                    ...
                    labelRef.label.text = this.createLabelText(
                        index,
                        positions,
                        Number(distance)
                    );
                    ...
                    labelRef.label.backgroundColor =
                        Cesium.Color.fromCssColorString('#3974F6'); // 背景色
                    labelRef.label.backgroundPadding = new Cesium.Cartesian2(
                        10,
                        10
                    ); // 边距
        ...
        [MeasureMode.Area]: ({
            ...
            this.activeMeasure.areaLabel.text = `面积:${areaUnit(
                Number(area)
            )}`;
            ...
            this.activeMeasure.areaLabel.backgroundColor =
                Cesium.Color.fromCssColorString('#3974F6'); // 背景色
            this.activeMeasure.areaLabel.backgroundPadding =
                new Cesium.Cartesian2(10, 10); // 边距
        ...
        [MeasureMode.DVH]: ({
            ...
            if (this.activeMeasure) {
                this.activeMeasure.disLabel.text = `空间距离:${distanceUnit(
                    Number(distance)
                )}`;
                this.activeMeasure.vLabel.text = `垂直高度:${distanceUnit(
                    Number(verticalHeight)
                )}`;
    ...
    async doExecute(
        ...
        const activeMeasure = new Cesium.MeasureHandler(
            this.BCS.viewer,
            mode,
            clampMode
        );
