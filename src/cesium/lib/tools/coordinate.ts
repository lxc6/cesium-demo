...
import { bbox, lineString, destination as Destination, distance as Distance } from '@turf/turf';
...
export const obtainCartesianNormal = (
    vector1: typeof Cesium.Cartesian3,
    vector2: typeof Cesium.Cartesian3
) =>
...
export function courseAngle(
    pointA: typeof Cesium.Cartesian3,
    pointB: typeof Cesium.Cartesian3,
 ...
 * @constructor
 */
export function cartesianToLatLng(position: typeof Cesium.Cartesian3) {
 ...
 * 将string经纬度转成其他格式
 */
export function stringToOtherFormatLatLng(rawCoordinates) {
 ...
 * */
export function getTargetLatLng(initialLatLng, givenLatLng, direction = 0): number[] {
    // 计算两点间的距离
    const distance = Distance(initialLatLng, givenLatLng, { units: 'degrees' });

    // 根据开始点,距离,方向,求出目标点
    const destination = Destination(initialLatLng, distance, direction, { units: 'degrees' });
