...
export function createStyle(baseUrl: string) {
    let link: HTMLLinkElement | null = document.querySelector('link.cesium-style');
...
// 创建一个基础三维场景
export async function createBaseCesiumView(container: Element, CESIUM_BASE_URL = '/cesium/') {
    window['CESIUM_BASE_URL'] = CESIUM_BASE_URL;
    await Promise.all([checkCesium(CESIUM_BASE_URL), createStyle(CESIUM_BASE_URL)]);
