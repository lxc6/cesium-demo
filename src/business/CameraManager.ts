...
export class CameraManager {
    ...
    public resetView(): Promise<void> {
        const { currentScene } = useCesiumStore.getState();
        if (!currentScene?.camera) {
            console.warn('未找到默认相机配置');
            return Promise.resolve();
        }

        return this.flyTo({
            position: {
                longitude: currentScene.camera.longitude,
                latitude: currentScene.camera.latitude,
                height: currentScene.camera.height,
            },
            heading: currentScene.camera.heading,
            pitch: currentScene.camera.pitch,
            roll: currentScene.camera.roll,
            duration: 2,
        });
    }
}
